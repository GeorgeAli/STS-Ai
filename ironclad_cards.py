        ...
        if not newstate.monsters[monster].is_gone:
            ...
            for attackmum in range(newstate.monsters[monster].move_hits):
                ...
                if newstate.player.block > 0:
                    if (
                        newstate.monsters[monster].block
                        < newstate.monsters[monster].move_adjusted_damage
                    ):
                        left = (
                            newstate.monsters[monster].move_adjusted_damage
                        ...
                        newstate.player.block -= newstate.monsters[
                            monster
                        ].move_adjusted_damage
                # lose hp if no block
                else:
                    newstate.player.current_hp -= newstate.monsters[
                        monster
                    ].move_adjusted_damage
...
def draw(gamestate, amount):
    ...
    can_draw = (
        True  # it is for "Battle Trance", which is cannot draw card for this turn
    )
...
def Burning_Pact(gamestate, cardindex, Upgrade):
    ...
    newstate = addcard(
        newstate,
        newstate.hand[cardindex].name,
        "exhaust_pile",
        newstate.hand[cardindex],
    )
...
def True_Grit(gamestate, hitmonster, Upgrade):
    ...
    if Upgrade:
        ...
        newstate = addcard(
            newstate, newstate.hand[x].card.name, "exhaust_pile", newstate.hand[x]
        )
